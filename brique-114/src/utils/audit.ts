...
export interface AuditLog {
  merchant_id: string;
  actor_id?: string;
  actor_type: "merchant" | "ops" | "sira" | "system" | "reviewer";
  action: string;
...
export async function logAudit(log: AuditLog): Promise<string> {
  try {
    // In production, this would write to molam_audit_logs table
...
export function getAuditContext(req: Request): {
  ...
  let actor_type: "merchant" | "ops" | "sira" | "system" | "reviewer" = "system";
  if (user) {
    if (user.roles.includes("sira_reviewer")) {
      actor_type = "reviewer";
    } else if (user.roles.includes("pay_admin") || user.roles.includes("compliance_ops")) {
      actor_type = "ops";
